...
class InitalBlock(nn.Module):
    def __init__(self, in_channels, use_prelu=True):
        ...
        self.prelu = nn.PReLU(16) if use_prelu else nn.ReLU()
...
class BottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels=None, activation=None, dilation=1, downsample=False, proj_ratio=4, 
        ...
        self.prelu1 = nn.PReLU() if use_prelu else nn.ReLU()

        if asymetric:
            self.conv2 = nn.Sequential(
                nn.Conv2d(inter_channels, inter_channels, kernel_size=(1,5), padding=(0,2)),
                nn.BatchNorm2d(inter_channels),
                nn.PReLU() if use_prelu else nn.ReLU(),
        ...
        self.prelu2 = nn.PReLU() if use_prelu else nn.ReLU()

        self.conv3 = nn.Conv2d(inter_channels, out_channels, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(out_channels)
        self.prelu3 = nn.PReLU() if use_prelu else nn.ReLU()

        self.regularizer = nn.Dropout2d(p_drop) if regularize else None
        self.prelu_out = nn.PReLU() if use_prelu else nn.ReLU()
...
class ENet(nn.Module):
    def __init__(self,cfg):
        ...
        self.initial = InitalBlock(cfg["in_channels"])
        ...
        # Stage 6
        self.fullconv = nn.ConvTranspose2d(16, cfg["num_classes"], kernel_size=3, padding=1,
                                            output_padding=1, stride=2, bias=False)
        initialize_weights(self)
        if cfg["freeze_bn"]: self.freeze_bn()
        if cfg["freeze_dropout"]: self.freeze_dropout()
